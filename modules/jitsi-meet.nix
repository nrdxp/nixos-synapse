...
let
  ...
  overrideJs = source: varName: userCfg:
    let
  ...
  config = mkIf cfg.enable {
    ...
    services.nginx = mkIf cfg.nginx.enable {
      ...
      virtualHosts.${cfg.hostName} = {
        ...
        locations."=/config.js" = {
          alias = overrideJs "${cfg.package}/config.js" "config"
            (recursiveUpdate defaultCfg cfg.config);
        };
        locations."=/interface_config.js" = {
          alias =
            overrideJs "${cfg.package}/interface_config.js" "interfaceConfig"
            cfg.interfaceConfig;
        };
      };
    };
    ...
    systemd.services.jicofo = let
      ...
      jicofoArgs = {
        "--host" =
          "localhost"; # disable org.jitsi.jicofo.ALWAYS_TRUST_MODE_ENABLED for nonlocalhost
        "--domain" = cfg.hostName;
    ...
    in {
      ...
      serviceConfig = {
        Type = "exec";
        ExecStart =
          "${cfg.jicofo.package}/bin/jicofo ${attrsToArgs jicofoArgs}";
    ...
    environment.etc."jitsi/jicofo/sip-communicator.properties".source =
      pkgs.writeText "sip-communicator.properties" (concatStringsSep "\n"
        (mapAttrsToList (k: v: "${k}=${v}") cfg.jicofo.config));
    environment.etc."jitsi/jicofo/logging.properties".source = mkDefault
      "${cfg.jicofo.package}/etc/jitsi/jicofo/logging.properties-journal";
